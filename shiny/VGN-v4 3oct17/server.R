...
shinyServer( function ( input, output ) {
  ...
  output$net.map <- renderForceNetwork({
    ...
    ego.list <- as.vector(input$ego.list)    # store any selected egos a vector of character names
    ...
    if( input$net.select != "Select a subnet" ) {        # if no subnetwork is chosen, skip all output to avoid an error message
      ...
      # only proceed if there is at least 1 link
      ...
      } # end of if-statement for having links
    } # end of if-statement for having selected a network
  }) # end of map output
